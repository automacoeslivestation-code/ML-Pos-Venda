...
class TelegramListener:
    ...
    def _processar_resposta(self, texto: str) -> int:
        ...
        try:
            self._postar_no_ml(interacao_id, pendente["tipo"], resposta_texto)
            self._memoria.adicionar(pendente["texto"], resposta_texto, pendente["intencao"])
            self._pendentes.remover(interacao_id)
            self._enviar_telegram(
                f"Resposta postada no ML e salva na base de conhecimento.\n"
